...
const deleteDatabaseTables = () => {
    return new Promise(resolve => {
        db.serialize(() => {
            ...
            db.run(`DROP TABLE IF EXISTS bowls_ordered`, (err) => {
                if (err) reject(err);
            });
            db.run(`DROP TABLE IF EXISTS bowls_stock`, (err) => {
                if (err) reject(err);
            });
...
const createDatabaseTables = () => {
    return new Promise(resolve => {
        db.serialize(() => {
            ...
            db.run(`CREATE TABLE IF NOT EXISTS bowls_ordered (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                orderId INTEGER,
                size TEXT NOT NULL,
                base TEXT NOT NULL,
            ...
            db.run(`CREATE TABLE IF NOT EXISTS bowls_stock (
...
db.run(`INSERT INTO bowls_stock(size, nrBowlsLeft, price, nrProteins, nrIngredients)
    VALUES 
    ("R", 10 ,9 , 1, 4),
    ("M", 8 ,11 , 2, 4),
    ("L", 6 ,14 , 3, 6);
